...
| [`[u]int[x]`](#uintx-type) |             Number of token to mint |
...
The function must be a `view`, have no `inputs`, and must return a unique output of type [`[u]int[x]`](#uintx-type).
...
This type can be any `int` (`int8`, ..., `int256`) or `uint` (`uint8`, ..., `uint256`).
