...
The `init` and `address` arguments are not required but is strongly advised. If the function doesn't implement it, it will be omitted when generating the user transaction.
