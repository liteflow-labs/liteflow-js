...
- Add fields `chainId`, `collectionAddress`, `tokenId` in type `CreateAssetTransactionPayload`
- Add fields `chainId`, `collectionAddress`, `tokenId` in query `orderFees`
- Add to mutation `refreshAsset` fields `chainId`, `collectionAddress` and `tokenId`
- Add field `quantity` to type `Asset` with filter and order by functionalities
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `AssetWatch`
- Add resolver `ownership(ownerAddress: String)` on type `Asset` to ease the fetch of a specific ownership
- Add resolver `groupedByDate` on type `AccountsConnection`
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Ownership`
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Offer`, `OfferOpenBuy`, `OfferOpenSale` and mutation `createOffer`
...
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Comment`
...
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `AssetHistory`
- Add fields `chainId`, `collectionAddress` and `tokenId` to query `ownership`
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Royalty`
- Add resolver `groupedByDate` on type `TradesConnection`
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Auction` and mutation `createAuction`
- Add resolver `groupedByDate` on type `CollectionsConnection`
- Add resolver `groupedByDate` on type `OffersConnection`
- Add fields `chainId`, `collectionAddress` and `tokenId` to mutations `watch` and `unwatch`
- Add resolver `groupedByDate` on type `AssetsConnection`
- Add fields `chainId`, `collectionAddress`, `tokenId` in mutation `createTransferAssetTransaction`
...
- Add fields `chainId`, `collectionAddress` and `tokenId` to type `Like`
